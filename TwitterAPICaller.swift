...
class TwitterAPICaller: BDBOAuth1SessionManager {    
    ...
    func retweet(tweetId: Int, success: @escaping() -> (),
        ...
        TwitterAPICaller.client?.post(url, parameters: ["id": tweetId], progress: nil, success: {(task: URLSessionDataTask, response: Any?) in success()}, failure: {(task: URLSessionTask?, error: Error)in
            failure(error)
        })
